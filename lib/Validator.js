 ...
 * `@empyria/common`'s `createValidator` — no Ajv, no Zod.
 *
 * `createValidator` throws a `EmpyriaError` on invalid input; this adapts
 * that into the provider's `{valid, data, errorMessage}` result shape.
 */
export const empyriaJsonSchemaValidator = {
	/**
