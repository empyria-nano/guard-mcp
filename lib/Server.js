...
import { empyriaJsonSchemaValidator } from './Validator.js'
 ...
 *   `@empyria/common`'s schema helpers (`string()`, `number()`, `bool()`, ...). Omit for a
 *   tool that takes no arguments. A property's own schema may additionally carry
 *   `optional: true` to exclude it from the tool's `required` list — see
 *   {@link defineActionSchema}; `@empyria/common`'s own `defineSchema` has no such concept
 *   (every declared property is required, even one with its own `default` — deliberately
...
/**
 * Builds the same JSON Schema shape as `@empyria/common`'s `defineSchema` — used here instead
 * of it directly because that function has no notion of an optional property: every key it's
 * given ends up in `required`, even one that already carries its own `default` (deliberately
 * preserved, not a gap — see `test/Server.test.js`'s "registers each action..." test, which
 * locks in exactly that: a `default`-bearing param is still `required` unless the action author
 * also opts it out explicitly). A property's own schema may carry `optional: true` to be
 * excluded from `required`; that marker is stripped before the property is embedded (it isn't a
 * real JSON Schema keyword, and must never leak into the tool's actual advertised inputSchema).
 * @param {Object<string, Object>} paramsDef - Map of param name to JSON schema, optionally
 *   carrying `optional: true`.
 * @returns {Object} `{ type: 'object', properties, required, additionalProperties: true }`.
 */
function defineActionSchema(paramsDef) {
	const properties = {}
	const required = []
	for (const [key, rawSchema] of Object.entries(paramsDef)) {
		const { optional, ...propertySchema } = rawSchema ?? {}
...
function registerService(server, service) {
	...
	for (const [actionName, action] of Object.entries(service.actions ?? {})) {
		...
		config.inputSchema = fromJsonSchema(
			defineActionSchema(params ?? {}),
			empyriaJsonSchemaValidator,
		)
 ...
 * `@empyria/common`'s `createValidator` (no Zod, no Ajv).
 *
