...
import { defineSchema } from '@empyria/common'
 ...
 *   `optional: true` to exclude it from the tool's `required` list — `registerService` passes
 *   `defineSchema`'s own `{ optional: true }` option (`@empyria/common` >= 0.1.4) to honor it.
...
function registerService(server, service) {
	...
	for (const [actionName, action] of Object.entries(service.actions ?? {})) {
		...
		config.inputSchema = fromJsonSchema(
			defineSchema(params ?? {}, { optional: true }),
